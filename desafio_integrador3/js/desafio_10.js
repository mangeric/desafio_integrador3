...
function pintar_carrito(carrito){
    ...
    for(let i=0; i<carrito.length;i++){
        $("#items").append(`<div id="template-carrito" class="card carro;">
            ...
            <p  id="precio${carrito[i].id}"  class="card-text">precio: ${carrito[i].precio*carrito[i].cantidad}</p>
            <p id="cantidad${carrito[i].id}" class="card-text ">cantidad: ${carrito[i].cantidad}</p>
            <button id="sumar${carrito[i].id}" class="btn btn-info btn-sm">
            +
            </button>
            <button id="restar${carrito[i].id}" class="btn btn-danger btn-sm">
                -
            </button>
...
function sumar(carro,btn){
    ...
    $(`#precio${carro.id}`).html(`precio:${carro.cantidad*carro.precio}`)
